...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (start == hope_to_rec) {
        ...
        if (have_written < len) {
            unassembled_slice.insert(
                make_pair(start + have_written, data.substr(start + have_written - index, len - have_written)));
        }
    } else {
    ...
    for (auto ptr = unassembled_slice.begin(); ptr != unassembled_slice.end();) {
        if (ptr->first == hope_to_rec) {
            ...
            if (have_written < (ptr->second).size()) {
                unassembled_slice.insert(make_pair(ptr->first + have_written,
                                                   data.substr(have_written, (ptr->second).size() - have_written)));
