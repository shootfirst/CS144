...
超文本传输协议，注意持续连接和非持续连接


### http报文

每一行都是回车符加换行符结束，最后一行多一个回车符

##### http请求报文

第一行是请求行，三个字段：方法字段、url字段和http版本字段，如GET /home.html HTTP、1.1

后面的行是首部行，格式一般是每一行一个首部字段加值，如Host：www.google.com

最后是实体行，存储传输的超文本内容

##### httop响应报文

第一行是状态行，三个字段：hhtp版本号、状态码和相应状态信息

后面是首部行，格式一般是每一行一个首部字段加值，如Connection：close，表示关闭tcp连接
...
+ 200 OK 一切正常
+ 204 No Content 和200一样，但是响应头没有body数据
+ 206 Partial Content 响应返回的 body 数据并不是资源的全部，而是其中的一部分，分块传输

##### 3xx

重定向，资源位置发生变动

+ 301 Moved PermanPermanently 请求的资源已经不存在了，需改用新的 URL 再次访问
+ 302 Found 表示临时重定向，说明请求的资源还在，但暂时需要用另一个 URL 来访问
+ 304 Not Modified 不具有跳转的含义，也就是告诉客户端可以继续使用缓存资源，用于缓存控制。
...
cpu收到网卡硬中断，跳转到异常处理e1000_intr，其首先禁止中断，然后将报文层层传递给对应的协议栈上层处理

内核中的ksoftirqd进程专门负责软中断的处理，当它收到软中断后，就会调用相应软中断所对应的处理函数，对于上面网卡驱动模块抛出的软中断，ksoftirqd会调用网络模块的
net_rx_action函数

net_rx_action调用网卡驱动里的poll函数来一个一个的处理数据包，在poll函数中，驱动会一个接一个的读取网卡写到内存中的数据包，内存中数据包的格式只有驱动知道

驱动程序将内存中的数据包转换成内核网络模块能识别的skb格式，经过系列处理，调用协议栈相应的函数，将数据包交给协议栈处理。
